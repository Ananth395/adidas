...
class RunSqlTestCase(ApplicationTestCase, RunSql):
    def test_read_sql_file(self):
        assert (
            RunSql._read_sql_file(
                dir_name=sql_dir, task_name="test_sql", table_name="sql"
            )
            == "test sql"
        )

    def test_create_temp_view(self):
        df = self.spark.createDataFrame(data=[[1, "ananth"]], schema=["id", "name"])
        RunSql._create_temp_view(df, view_name="test_view")

        expected_df = self.spark.createDataFrame(
            data=[[1, "ananth"]], schema=["id", "name"]
        )

        assert (
            self.spark.sql("select * from test_view").collect() == expected_df.collect()
        )

    def test_run_sql(self):
        sql = "select year(current_date) as year"
        df = self._run_sql(sql)
        expected_df = self.spark.createDataFrame(data=[[2021]], schema=["year"])
